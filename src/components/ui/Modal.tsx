...
export function Modal({
  ...
  children: ReactNode
  wide?: boolean
  /** Overrides `wide`. 'xl' is for review grids that need the room. */
  size?: keyof typeof widths
}) {
  return (
    <div className="modal-overlay fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
      <div
        className={`flex max-h-[90vh] w-full ${widths[size ?? (wide ? 'wide' : 'md')]} flex-col rounded-xl border border-gold-400/30 bg-white shadow-lg dark:border-gold-500/20 dark:bg-slate-800`}
      >
